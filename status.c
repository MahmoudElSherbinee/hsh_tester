...
{
	int isStdinTerminal, isReadFdValid, isInteractive;

	/* Step 1: Check if the standard input is associated with a terminal */
	isStdinTerminal = isatty(STDIN_FILENO);

	/* Step 2: Check if the 'file_descriptor' member */
	/* is within the standard input range */
	isReadFdValid = (information->file_descriptor <= 2);

	/* Combine the results of both checks using logical AND */
	isInteractive = isStdinTerminal && isReadFdValid;
